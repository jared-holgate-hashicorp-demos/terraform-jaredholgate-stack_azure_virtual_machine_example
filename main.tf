locals {
    ...
    consul_ip_addresses = [ for index in range(10, var.consul_cluster_size + 11) : cidrhost(local.subnets[0], index) ]
    vault_ip_addresses = [ for index in range(10, var.vault_cluster_size + 11) : cidrhost(local.subnets[1], index) ]
